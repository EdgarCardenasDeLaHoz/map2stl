...
def main():
    ...
    ap.add_argument("--smart-init-jitter", type=float, default=0.0,
    ...
    for cycle in range(1, args.cycles + 1):
        ...
        if overfit:
            ...
            if can_deepen:
                ...
                # Grow only the highest-scoring block to keep distribution shift small.
                # Growing all blocks at once often causes a larger short-term score drop.
                new_channels = list(channels)
                new_channels[best_idx] = channels[best_idx] + max(1, args.grow_channels)
                print(f"    [growing block {best_idx} +{max(1, args.grow_channels)}: {channels} → {new_channels}]")
                cycle_record["decision"] = f"grow_block_{best_idx}→{new_channels}"
