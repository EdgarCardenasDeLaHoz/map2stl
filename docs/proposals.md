...
|----|-------------|---------|--------|--------|
| F-MESHIMPORT | Import STL/OBJ as a registered layer — upload endpoint + `stl_to_heightmap`/`infill_idw` wiring (previously notebook-only), a manual side-by-side control-point registration UI (2D affine fit from clicked point pairs, since the existing automatic edge-IoU registration in `numpy2stl.registration` "has not worked well"), a new `MeshImport` stacked layer, and an "Apply to DEM" merge action mirroring `composite-dem.js`. Also added a mesh library browser for pre-existing STL sets (e.g. commercial city packs) with per-city location sidecars + disk-cached heightmaps (`config.MICROPOLITAN_STL_DIR`, env `STRM2STL_MICROPOLITAN_DIR`). Fixed a latent `stl_to_heightmap` bug found along the way (NaN-accumulator always returned empty heightmaps, see `docs/issues.md`) and added a grid-size guard (`MAX_DIM`) so region-sized bboxes at fine resolution fail fast instead of hanging on a 100M+ pixel ray-cast. **Same-day extension**: added an "Auto" mode (geocode filename → `numpy2stl.registration.pipeline.register_city_stl` → bbox-overlap region match/create → opens the manual picker pre-filled for confirmation, never auto-accepted). Along the way, found and fixed two real bugs in `numpy2stl/registration/align/global_search.py` (scale selection was picking a flat/noisy xcorr-peak-at-search-boundary instead of the sharp Dice-peak; rotation refinement let noisy full-frame edge-IoU override an already-correct histogram estimate on dense-grid cities like Miami) — both validated against real Miami STL data plus 2 new targeted regression tests, zero regressions across numpy2stl's 159-test suite. User-requested 2026-07-19. Plan: [plans/F-MESHIMPORT-stl-obj-layer-import.md](plans/F-MESHIMPORT-stl-obj-layer-import.md). | `app/server/routers/layers.py`, `app/server/core/mesh_import.py`, `app/server/routers/regions.py` (region matching), `app/client/static/js/modules/layers/mesh-layer.js`, `app/client/static/js/modules/layers/mesh-registration.js`, `app/client/static/js/vue/components/dem/MeshImportSection.vue`, `app/client/static/js/vue/components/layout/MeshRegistrationModal.vue`, `../numpy2stl/registration/align/global_search.py`, `stacked-layers.js`, `LayerViewSection.vue`, `tests/test_mesh_import.py`, `tests/test_region_matching.py` (new), `tests/e2e/test_mesh_import.py`, `../numpy2stl/tests/test_registration_register.py` | Large | done (2026-07-19) |
| F-PROJ-EXPAND | Expand map projections to cover the MapChart guide set (Miller, Gall Stereographic, Mollweide, Robinson, Eckert IV, Winkel Tripel, Van der Grinten I, Times). Phased by pipeline risk: cylindrical (drop-in) → pseudocylindrical (2D-warp path) → azimuthal/polyconic (biggest departure). User-requested 2026-07-19, goals: real-space accuracy + visual variety + distortion teaching. Plan: [plans/F-PROJ-EXPAND-map-projections.md](plans/F-PROJ-EXPAND-map-projections.md). | `geo2stl/projections.py`, `app/server/routers/settings.py`, `app/client/static/js/vue/components/dem/DemSourceSection.vue`, `app/client/static/js/modules/dem/dem-gridlines.js` | Large | in-progress |
...
| F-CLEAN14 | Split the three over-large skyline files into focused modules — verbatim moves + re-exports, no behaviour change. **Phase H done** (html_report 2882→1217 + new report_plots 1710). **Phase R done** (region_pdf 6524→698; new region_types/region_config/region_data/streetview_io/seed_selection/region_render/pano_registration). Surfaced + fixed a latent `logger` NameError. **Full package split done (2026-06-23):** `pipeline.py`→`_core/`, `pano_registration.py`→`_pano/`, `report_plots.py`/`region_render.py`→`_report_plots/`/`_region_render/` — façades preserve all import paths; 773 tests + 10 e2e green; ruff/F821 audit fixed 3 NameError bugs on untested paths. Plan: [plans/F-CLEAN14-skyline-file-split.md](plans/F-CLEAN14-skyline-file-split.md). | `city2stl/skyline/*.py` | Large | ✅ done — full split (2026-06-23) |
| F-DET | Detection quality & early-out: F-DET1 blob-count pano screen (early-out < 6 blobs), F-DET2 OSM-FOV proposal gate, F-DET3 weak sub-labels, F-DET5 landing-page det column — all done; F-DET4a/b/c (Type 2 per-city fixes) pending, assumptions challenged 2026-06-23. Plan: [plans/F-DET-detection-quality-and-early-out.md](plans/F-DET-detection-quality-and-early-out.md). | `city2stl/skyline/pano_registration.py`, `seed_selection.py`, `html_report.py`, `scripts/build_landing_page.py` | Medium | in-progress — 1/2/3/5 done, 4a–c pending |
| F-COMPOSITE2 | Composite DEM rebuild: (1) DEM base itself becomes a toggleable/weighted contribution like the other channels; (2) City/OSM group split into independent buildings/roads/waterways/walls toggles+weights instead of one combined cityWeight; (3) per-layer + combined mini histograms (canvas-drawn) in the composite panel; (4) two-tier city size gate — existing 10km detailed per-building tier unchanged, new 25km coarse tier fetching only major roads + water + large buildings (area-thresholded, no walls/small-building detail); (5) basic side-by-side split view (composite \| satellite), independently pannable/synced. User-requested 2026-07-19, root cause of the "SF only shows tree height" report was CITY_MAX_DIAG_KM=10 silently skipping buildings for the ~18km test bbox. Plan: [plans/F-COMPOSITE2-rebuild.md](plans/F-COMPOSITE2-rebuild.md). | `app/client/static/js/modules/layers/composite-dem.js`, `app/client/static/js/modules/layers/city-overlay.js`, `app/client/static/js/modules/layers/stacked-layers.js`, `app/client/static/js/vue/components/dem/CompositeDemSection.vue`, `app/server/routers/cities.py`, `app/server/core/validation.py`, `app/server/config.py` | Large | done (2026-07-19) |
| F-PROJ-DIMS | Variable-dimension projection output — flip `maintain_dimensions` default to `False` across the whole pipeline so switching projections visibly changes the rendered aspect ratio to the true geographic shape, instead of every projection being silently forced back to the unprojected input's `(m,n)`. Fixes broken/no-op `False` branches in `_project_cosine` (unsafe per-row remap), `_project_sinusoidal`/`_project_cylindrical_y` (miller/gall — ignored the flag). User-requested 2026-07-19 as a full one-pass implementation (no deferred phases) after auditing why projection changes never visibly changed canvas aspect ratio. `projection='none'` intentionally stays equal-angle (not equal-distance) per user decision — just gets a clarifying UI note. Plan: [plans/F-PROJ-DIMS-variable-projection-output.md](plans/F-PROJ-DIMS-variable-projection-output.md). | `geo2stl/projections.py`, `app/server/routers/terrain.py`, `app/server/routers/composite.py`, `app/server/routers/cities.py`, `app/server/core/export.py`, `app/server/schemas.py`, `app/client/static/js/vue/components/dem/ProjectionSection.vue`, `app/client/static/js/modules/core/ui-helpers.js` | Large | done (2026-07-19) |
