...
def write_region_report(
    ...
    estimates_by_seed = estimates_by_seed or {}

    # Dedupe by seed name (seed_views has one entry per view)
    seen_seeds: dict[str, "SeedViewRegistration"] = {}
    for sv in seed_views:
        seen_seeds.setdefault(sv.seed_name, sv)

    for seed_name, sv in seen_seeds.items():
        png_path = minimap_dir / f"{seed_name}.png"
        ok = _render_seed_minimap_png(png_path, sv, osm_data, buildings_by_id)
        rel = f"assets/minimap/{seed_name}.png" if ok else None
    ...
    logger.info(
        "F-SKY15 HTML report written: %s (%d seeds)",
        out_dir, len(seen_seeds),
    )
