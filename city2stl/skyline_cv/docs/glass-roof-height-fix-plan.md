...
Tall glass / curtain-wall towers under-predict by 50–100 m in `skyline_cv` output (see [STATUS.md](../STATUS.md#heights-the-core-product-gap)). Three live hypotheses, none ruled out:
...
A new tool: `city2stl/skyline_cv/scripts/09_height_trace.py`.

```bash
python city2stl/skyline_cv/scripts/09_height_trace.py \
...
|---|---|---|
| `city2stl/skyline_cv/depth.py` | `compute_depth_map(image) -> np.ndarray`; LRU cache mirroring `_segformer_cache`; `roof_y_from_depth(depth, x_range, sky_threshold)`; `sky_threshold_from_depth(depth, sky_mask) -> float` (uses the SegFormer sky_mask to pick a robust threshold) | ~250 |
| `tests/test_skyline_cv_depth.py` | unit tests for `sky_threshold_from_depth`, `roof_y_from_depth`, cache anchoring | ~150 |
...
4. **Tests stay green.** All 21 existing skyline_cv tests still pass; new depth tests pass.
