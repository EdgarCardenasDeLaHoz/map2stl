...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--out-dir",
        default=None,
        help="Output directory (default: city2stl/skyline/runs/height_traces)",
    )
...
def main() -> int:
    ...
    out_dir = (
        Path(args.out_dir)
        if args.out_dir
        else ROOT / "city2stl" / "skyline" / "runs" / "height_traces"
    )
    ...
    if not args.no_render:
        try:
            from city2stl.skyline.height_trace_render import (  # noqa: PLC0415
