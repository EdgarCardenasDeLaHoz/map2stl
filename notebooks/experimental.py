...
def fill_building_heights(gdf):
		...
		if 'building:levels' not in gdf.columns:
			...
			gdf['building:levels'] = pd.to_numeric(gdf['building:levels'], errors='coerce').fillna(3) 
...
def add_building_z(gdf, im, coor_lims):
...
def reduce_buildings(gdf, TOLERANCE_M=1, a=1):
		...
		gdf_optimized = gdf_optimized[~gdf_optimized.is_empty & gdf_optimized.is_valid]
